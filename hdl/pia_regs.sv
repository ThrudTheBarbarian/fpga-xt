...
module pia_regs (
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            unique case (wreg)
                ...
                2'b01: begin
                    // PORTB writes ALWAYS update the output latch —
                    // 130XE banking semantics rely on that. The DDR
                    // mode still tracks DDRB if PBCTL[2]=0 (kept for
                    // OS-level compatibility), but the output latch
                    // gets the wdata regardless.
                    portb_out_latch_q <= wdata;
                    if (!pbctl_q[2]) portb_ddr_q <= wdata;
                end
                2'b10: pactl_q <= wdata;
                2'b11: pbctl_q <= wdata;
            endcase
    ...
    always_comb begin
        ...
        if (read_in_win) begin
            unique case (rreg)
                2'b00: rdata = pactl_q[2] ? joy_porta_in : porta_ddr_q;
                2'b01: rdata = pbctl_q[2] ? joy_portb_in : portb_ddr_q;
                2'b10: rdata = pactl_q;
                2'b11: rdata = pbctl_q;
            endcase
        end
