...
// compositor talk to either the system-RAM shadow (HyperRAM-backed
// in the M16b-int+ flow) or dma_master (multi-cycle 6502-bus master)
// without caring which is providing the data.
//
...
//
// History: pre-M16b-int the shadow side was a 1-cycle BRAM with no
// handshake (sh_ready was implicit). Replacing it with HyperRAM
// added multi-cycle latency, which propagates here via sh_ready /
// sh_req. Tests that wire a BRAM into this adapter pin sh_ready=1.
...
module mem_read_mux #(
    ...
    // Shadow (system-RAM) side. With hyperram_shim this is its
    // req/ready/rdata read-port handshake. With a BRAM (in tests),
    // sh_ready is tied 1.
