...
module color_resolver (
    ...
    // ANTIC text modes 2/3 (GR.0): GTIA takes the character LUMA from COLPF1
    // but the HUE from COLPF2 — COLPF1 only sets text brightness, not colour.
    // When asserted, the COLPF1 playfield colour becomes {COLPF2 hue, COLPF1
    // luma}.  Off for every other mode (COLPF1 used verbatim).
    ...
    // GR.0 (mode 2/3) COLPF1 luma-only substitution: hue from COLPF2, luma
    // (low nibble) from COLPF1.  Identity when the flag is low.
    wire [7:0] colpf1_eff = colpf1_luma_only ? {colpf2[7:4], colpf1[3:0]} : colpf1;
    ...
    always_comb begin
        ...
        else if (pf1)     pf_color_normal = colpf1_eff;
    ...
    always_comb begin
        ...
        pf_hi_color_normal   = pf0 ? colpf0 : (pf1 ? colpf1_eff : 8'h0);
