...
## Cold boot lands in the self-test instead of BASIC — OPEN

Closing the emulator window (or `xlboot -e`, or `6502 basic`) comes up in the XL
memory self-test (solid green, `PC=$505x`) instead of BASIC.  **Sticky, not rare:**
once green, always green (8/8 uninstrumented tight ejects).  Blue only ever happens
before a game has run in that power cycle.  Simon also hears the "disk not there"
buzz — that is the failed boot attempt on the way to the self-test.

**Route, measured:** `$BFFC` (CART) reads **$BF**, not the BASIC ROM's `$00`, so
`$C3D2 BNE PRS22` = "no cartridge" → no BASIC → coldstart jumps DOSVEC(=PPD `$F223`)
→ SES (`$C8FC`) → writes `$7D` to PORTB (self-test ROM in; caught at `PC=$C909`) →
runs the self-test.

**`$A000-$BFFF` reads as OPEN BUS.** `$BFFC` and `$BFFD` both return `$BF` = the
address high byte, and the value is *insensitive to PORTB in both directions*
(banked in `$FD` and banked out `$FF` both give `$BF`).  Nothing in the memory
system claims that window on this path; `$BF` is the CPU's float, not a mux leg —
no open-bus generator returning `addr[15:8]` exists in `sally_mem`.

**Eliminated, each by measurement — do not revisit:**
- NGFLAG / RAM test / ROM checksums: breakpoints at `C2EC`, `C2F6`, `C31D` never hit.
- PIA DDR-mode confusion: `BIT $D303` with `A=$04` at the cart check returns **Z clear**
  ⇒ PBCTL[2]=1 ⇒ **port mode**.  (A `$D301` read-back cannot distinguish the modes —
  port mode returns `out`, DDR mode returns `portb_ddr_q`, and both are `$FD`.)
- PORTB write history is healthy (`6502 watch \$D301 w`): `$FF`@C4FD, `$FF`@C50F,
  `$FF`@C48F, **`$FD`@C4A9 (BASIC on)**, `$7D`@C313, **`$FD`@C327 (restored)**.
  No `$D301` write between icnt 804771 and the cart check at 805297.
- Re-running the OS's own `$C4A1 LDA PORTB/AND #$FD/STA PORTB` then re-executing
  `LDX CART` still yields `$BF`.
- Port-mode arithmetic forces `DDRB=$FF` and `portb_out_latch_q=$FD`, so
  `basic_rom_en` *should* assert.  One synthesized `pia_regs` only (`antic_top:1889`);
  `sally_mem.portb` = `antic_top.portb_q` = that latch, direct, no gating or CDC.
- External cart (`bus_rd5_n_in` tied `1'b1`), screen banking (`$4000-$5FFF` aperture
  only), the ROM image (`atari-basic.rom` has `$BFFC=$00`, init `$BFF0`, flags `$05`).
- Entry point: `6502 basic` (direct `xl_reset(1)`) fails identically to `xlboot -e`.
- Mid-boot corruption: CART is already `$BF` at icnt 223630, the earliest point after
  PMI enables BASIC.
- Upload coverage: `upload_image` writes `$1000-$CFFF` + `$D800-$FFFF` every time, and
  `mem_we = cpu_w || rom_we` — the ROM-window write is **not** gated by `rom_override`.
  `romwin_write` is correct C (`(uint32_t)a + i` cannot wrap).

**The live discriminator:** the same `$BFFC` reads **`$00`** when the machine is booted
via the XEX path (`6502 run /peek.xex`, which forces `PORTB=$FD`, loads `$BFFC` and
...
**Instrument traps (these invalidated a whole session's measurements):**
1. `ssh root@board '6502 break $C3CF'` — the **board's** shell expands `$C3CF` to
   empty, so the breakpoint arms at `$0000` and never hits.  Write `\$C3CF`.  Validate
   with `6502 break \$C2D6`, which must hit on every cold start.
2. Breakpoints halt **before** executing the instruction at PC; watchpoints halt
   **after** the access (PC = next instruction).  To read an instruction's result,
   break at the next address.
3. Always `6502 break off; 6502 watch off; 6502 breakreset off; 6502 go` between
   tests — otherwise the next reset comes up halted at `PC=$FFFC icnt=0` and the test
   is silently invalid.
4. `6502 step` is confounded by the VBI NMI (SP drops by 3, PC lands in `$C01x`).  Use
   breakpoints plus `6502 PC=\$xxxx go`.
5. The debugger accepts register assignment (`6502 PC=\$C3CF A=\$04 go`) — this is how
   to use the CPU itself as a memory probe when there is no peek/poke verb.  `BIT $D303`
   at `$C0AD` with `A=$04` reads PBCTL bit 2.
6. Board `/tmp` is a tiny ramdisk: it fills silently and then every write fails with
   "No such file or directory".  Write to the SD root instead.
