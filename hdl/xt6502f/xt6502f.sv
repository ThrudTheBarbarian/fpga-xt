...
module xt6502f #(
    ...
    reg       nmi_d1, nmi_d2;  // same 2-cycle setup rule for NMI recognition (ACID800
                               ...
                               // boundary).  The BRK-hijack window keeps the LIVE pend.
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            // Interrupt model, calibrated against ACID800 dlitiming (even,
            // odd, delayed) + pokey_irqtiming:
            //   * d1 samples the line/pend at EVERY commit slot of the
            //     free-running sub counter (wall machine cycles, RDY or
            //     not) — the NMOS samples the pins every phi2.
            //   * the POLL latch samples d1 only at RDY-TRUE commit slots —
            //     the poll needs the CPU clocking, so a WSYNC stall FREEZES
            //     the decision: an edge arriving mid-stall cannot claim the
            //     stalled instruction's own boundary and is taken one
            //     instruction after the resume (the 'delayed' cases).
            //   * when running, polled(B) = pend(B-2): the 2-cycle setup
            //     rule (plain even/odd cases + IRQEN-delay contract).
            if (sub == SUB_COMMIT[7:0]) begin
                ...
                if (rdy) begin
                    irq_polled  <= irq_d1;
                    nmi_polled  <= nmi_d1;
                    // NMI and IRQ take the SAME two-stage rule -- emu polls at
                    // the end of every cycle and decides on the penultimate
                    // sample, for both.  A third NMI stage used to compensate
                    // for the WSYNC release landing a cycle late in antic2;
                    // with the release exact it made the marginal alignments
                    // (dlitiming odd / phase-2/3) one instruction late.
                end
            end
          if (dbg_load) begin
            ...
            case (state)
                ...
                ST_BRK4: begin S <= S - 8'd1; P[2] <= 1'b1; i_poll <= 1'b1;  // push P; S--; set I (poll-mask too, immediately)
                    ...
                    if (nmi_d1) begin nmi_svc <= 1'b1; nmi_pend <= 1'b0;     // BRK/IRQ -> NMI vector hijack
                        ...
                        if (!intr) begin nmi_d1 <= 1'b0; nmi_polled <= 1'b0; end
                    end
                    state <= ST_BRK5; end
                ...
                ST_BRK5: begin eal <= din_r;
                    if (!intr) begin nmi_pend <= 1'b0; nmi_d1 <= 1'b0; nmi_polled <= 1'b0; end
