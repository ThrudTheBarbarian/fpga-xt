...
## Verification — DONE on hardware 2026-07-29

Confirmed with `/tmp/rngtest.so` on the board:

```
getrandom(GRND_NONBLOCK) = 32   -> HARDWARE-BACKED
getrandom(blocking)      = 32
two draws differ (ok)
TRNG_STAT before = 0x00000120  (fresh=1 bits=32)
TRNG_RND read    = 0xcadc48e1
TRNG_STAT after  = 0x00000013  (fresh=0 bits=19)
```

The STAT transition is the part that matters: `0x120` -> read -> `0x013`
proves **read-to-consume** works — the counter restarted rather than staying
saturated. This could NOT be shown from the shell (the pool refills in ~1 us,
far faster than two `mem` commands), so a tight C loop was the only way.

It also corroborates the refill rate the whole gating design rests on: 19 bits
accumulated in the gap between the two reads, and at ~33 Mbit/s post-von-Neumann
19 bits is ~0.6 us — consistent with two back-to-back register reads. That
figure was calculated until now; it is measured.

## Original verification note
