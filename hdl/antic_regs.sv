...
module antic_regs (
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            output_mode  <= 8'h00;      // OUTPUT_MODE: legacy compositing (output is always 1080p60)
            pal_r        <= 8'h00;
            pal_g        <= 8'h00;
            pal_b        <= 8'h00;
            pal_idx      <= 8'h00;
            ...
            if (cold_boot) begin
                ...
                unique case (waddr[6:0])
                    ...
                    7'h01: begin                       // $D481 MODE
                        // mode_snoop is reset-locked to 1 (snoop) and NOT
                        // bus-writable: $D481 is a stock ANTIC mirror of $D401
                        // (CHACTL) — real ANTIC aliases $D400-$D4FF onto its 16
                        // regs — so the legacy XL OS writes $D481<=$00 during
                        // display init, which would clobber mode_snoop -> DMA
                        // mode.  On the Zynq build the DMA master is tied off
                        // so snoop is the ONLY valid mode; ignore the bus write
                        // (same as $D480 CLOCK_MULT above).  ANTIC then reads
                        // the DLIST/screen from sally_mem and renders correctly.
                        // mode_snoop       <= wdata[0];   // intentionally locked
