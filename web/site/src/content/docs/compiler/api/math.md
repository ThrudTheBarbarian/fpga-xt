...
`Math` is reimplemented per-architecture (`support/xt6502/lib/Math.xt`, `support/arm64/lib/Math.xt`, …). The API is the same everywhere — same overloads, same constants — and because the xtc `float` and `double` formats are defined by the language rather than the host, the bit-level layout of values is identical across targets. Only the helper routines differ.
