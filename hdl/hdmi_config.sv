...
module hdmi_config (
                        ...
                        if (i2c_ready) begin
                            ...
                            case (sub_op)
                                ...
                                3'd1: begin
                                    ...
                                    i2c_data_i    <= 8'h72 << 1;  // 0xE4 = addr|W
                                ...
                                3'd4: begin
                                    i2c_cmd_valid <= 1'b1;
                                    i2c_cmd_op    <= 2'd2;   // WRITE
                                    i2c_data_i    <= (8'h72 << 1) | 1'b1;  // 0xE5 = addr|R
                                    sub_op        <= 3'd6;
                                end
    ...
    function automatic [16:0] seq_rom_read(input [4:0] index);
        case (index)
            5'd0:  return {1'b0, 8'hC7, 8'h00};  // Enable TPI mode
            5'd1:  return {1'b0, 8'h1E, 8'h00};  // D0 power state
            5'd2:  return {1'b0, 8'hBC, 8'h01};  // Indexed: page 0
            5'd3:  return {1'b0, 8'hBD, 8'h82};  // Indexed: reg 0x82
            5'd4:  return {1'b0, 8'hBE, 8'h01};  // Source termination enable (bit[0]=1)
            5'd5:  return {1'b0, 8'h00, 8'hFC};  // PixelClock LSB (14844)
            5'd6:  return {1'b0, 8'h01, 8'h39};  // PixelClock MSB
            5'd7:  return {1'b0, 8'h02, 8'h3C};  // VFreq LSB (60)
            5'd8:  return {1'b0, 8'h03, 8'h00};  // VFreq MSB
            5'd9:  return {1'b0, 8'h04, 8'h98};  // H_Total LSB (2200)
            5'd10: return {1'b0, 8'h05, 8'h08};  // H_Total MSB
            5'd11: return {1'b0, 8'h06, 8'h65};  // V_Total LSB (1125)
            5'd12: return {1'b0, 8'h07, 8'h04};  // V_Total MSB
            5'd13: return {1'b0, 8'h08, 8'hB0};  // Input bus: full pixel, x1, rising
            5'd14: return {1'b0, 8'h1A, 8'h00};  // System: TMDS active (bit4=0), DVI
