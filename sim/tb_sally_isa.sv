// tb_sally_isa.sv — focused ISA tests for the xt6502 core (SALLY embellishments).
//
// Boots xt6502 + a flat 64 KB memory model (no sally_mem, no
...
// from xt6502's widened SP land at the same logical location as
...
module tb_sally_isa;
    ...
    task automatic test_add_sp_cross_boundary();
        ...
        // absolute store (xt6502 in this TB writes to the flat mem
    ...
    //   subtracts N+7 = 7 from SP (1 guard byte + 6 saved), then writes:
    //     stack[SP+1] = P, stack[SP+2] = SP_lo_old ($FF),
    //     stack[SP+3] = SP_hi_old ($0F), stack[SP+4] = Y ($CC),
    //     stack[SP+5] = X ($BB), stack[SP+6] = A ($AA); SP+0 = guard.
    //   New SP = $FF8, so the saved slots land at mem[$0FF9..$0FFE]
    //   (same absolute addresses as before; SP itself is one lower).
    ...
    task automatic test_psh_save_slots();
        ...
        // SP_lo after PSH = $FFF - 7 = $FF8 -> low byte $F8 (guard byte).
        expect_mem(16'h0330, 8'hF8, "PSH #0: TSX returns $F8");
    ...
    task automatic test_psh_pll_locals();
        ...
        // Inside frame: SP=$FF4 (N+7 = guard byte + 6 saved + 4 locals).
        // Local[0] lives at SP+7 = $FFB ($0FF4 + 7 = $0FFB).  Store $5A there.
        put(16'h0408, 8'hA9); put(16'h0409, 8'h5A);             // LDA #$5A
        put(16'h040A, 8'h92); put(16'h040B, 8'h07);             // STA $07,SP
        // Confirm round trip from same SP:
        put(16'h040C, 8'hA9); put(16'h040D, 8'h00);             // LDA #$00
        put(16'h040E, 8'hB2); put(16'h040F, 8'h07);             // LDA $07,SP
    ...
    task automatic test_calling_convention();
        ...
        //   PSH #0 frame layout (offsets from new SP — guard byte at +0):
        //     +0 guard  +1 P  +2 SP_lo  +3 SP_hi  +4 Y  +5 X  +6 A
        //     +7 local-gap  +8 ret_lo  +9 ret_hi  +10 arg0 (a)  +11 arg1 (b)
        put(16'h0420, 8'h32); put(16'h0421, 8'h00);             // PSH #0
        put(16'h0422, 8'hB2); put(16'h0423, 8'h0A);             // LDA $0A,SP (= a = $22)
        put(16'h0424, 8'h18);                                   // CLC
        put(16'h0425, 8'h72); put(16'h0426, 8'h0B);             // ADC $0B,SP (+= b = $14)
        put(16'h0427, 8'h92); put(16'h0428, 8'h06);             // STA $06,SP (overwrite saved-A
                                                                //              with return value)
    ...
    // ---- Test 30: PSH guard byte — a push in a live frame must not corrupt
    //   the saved registers (regression for Issue #0001).  SEC sets C=1; PSH
    //   saves P (C=1).  A PHA #$00 inside the frame must land on the guard
    //   byte (SP+0), NOT on saved P (SP+1).  PLL restores P; if the guard byte
    //   is missing the PHA overwrote saved P with $00 (C=0) and the restored
    //   C is wrong.  ADC #$00 then materialises C: $01 = guard works.
    task automatic test_psh_guard_byte();
        $display("=== Test 30: PSH guard byte (push in live frame) ===");
        reset_cpu();
        mem[16'hFFFC] = 8'h00;
        mem[16'hFFFD] = 8'h04;
        prologue();
        put(16'h0404, 8'h38);                                   // SEC (C=1)
        put(16'h0405, 8'h32); put(16'h0406, 8'h00);             // PSH #0 (saves P, C=1, at SP+1)
        put(16'h0407, 8'hA9); put(16'h0408, 8'h00);             // LDA #$00
        put(16'h0409, 8'h48);                                   // PHA  → guard byte SP+0
        put(16'h040A, 8'h68);                                   // PLA  → SP restored
        put(16'h040B, 8'h18);                                   // CLC (C=0)
        put(16'h040C, 8'h62); put(16'h040D, 8'h00);             // PLL #0 (restore P → C should be 1)
        put(16'h040E, 8'hA9); put(16'h040F, 8'h00);             // LDA #$00
        put(16'h0410, 8'h69); put(16'h0411, 8'h00);             // ADC #$00 (A = C)
        put(16'h0412, 8'h8D); put(16'h0413, 8'h64); put(16'h0414, 8'h03);  // STA $0364
        put(16'h0415, 8'h4C); put(16'h0416, 8'h15); put(16'h0417, 8'h04);  // JMP $0415
        run_until_stuck(16'h0415, 600, "PSH guard byte");
        expect_mem(16'h0364, 8'h01,
                   "guard byte: PHA in live frame left saved P (C=1) intact");
        $display("PASS: test_psh_guard_byte");
    endtask
