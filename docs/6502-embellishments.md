...
grows from 5 to 6 bytes; `frame_size = N + 6`.
...
```
frame_size = N + 6                     ; reg save (6) + locals (N)
SP = SP − frame_size                   ; 1. allocate full frame

stack8(SP + 5) = A                     ; 2. write saved A
stack8(SP + 4) = X                     ; 3. write saved X
stack8(SP + 3) = Y                     ; 4. write saved Y
stack8(SP + 2) = SP_hi_entry           ; 5. write saved SP[11:8] (entry value)
stack8(SP + 1) = SP_lo_entry           ; 6. write saved SP[7:0]  (entry value)
stack8(SP + 0) = P                     ; 7. write saved P
; SP unchanged by writes — still points below frame
```
...
higher addresses
  ...
  │    saved X           │  ← at SP + 4
  │    saved Y           │  ← at SP + 3
  │    saved SP[11:8]    │  ← at SP + 2
  │    saved SP[7:0]     │  ← at SP + 1
  │    saved P           │  ← at SP + 0
  ├─────────────────────┤
  │    (free /           │
  │     interrupt safe)  │
...
The gap byte at `+N + 6` exists because the 6502's `JSR` pushes two bytes
...
local area is the N bytes at `+6..+N+5`.
...
```
SP after PSH = $FFB − 12 − 6 = $FE9

$FE9:  saved P           ← SP + 0
$FEA:  saved SP_lo       ← SP + 1
$FEB:  saved SP_hi       ← SP + 2  (holds $0F = SP[11:8] of entry $FFB)
$FEC:  saved Y           ← SP + 3
$FED:  saved X           ← SP + 4
$FEE:  saved A           ← SP + 5
$FEF:  local[0]          ← SP + 6
 ...
$FFA:  local[11]         ← SP + 17  = SP + 6 + 11
$FFB:  (gap)             ← SP + 18  = SP + N + 6
$FFC:  ret_lo            ← SP + 19  = SP + N + 7
$FFD:  ret_hi            ← SP + 20  = SP + N + 8
$FFE:  param_hi          ← SP + 21  = SP + N + 9
$FFF:  param_lo          ← SP + 22  = SP + N + 10
```
...
```
P     = stack8(SP + 0)                 ; 1. read saved P
SP_lo = stack8(SP + 1)                 ; 2. read saved SP[7:0]  (see note)
SP_hi = stack8(SP + 2) & $0F           ; 3. read saved SP[11:8] (see note)
Y     = stack8(SP + 3)                 ; 4. read saved Y
X     = stack8(SP + 4)                 ; 5. read saved X
A     = stack8(SP + 5)                 ; 6. read saved A
SP    = SP + (N + 6)                   ; 7. deallocate full frame
```
...
holds if pushes and pops are balanced).  The final `SP += (N + 6)`
...
```
    LDA  +(N + 7),SP      ; load low byte of first param
    LDA  +(N + 8),SP      ; load high byte of first param
```

`N` = locals_size.  The return address occupies 2 bytes above the
locals, and the gap byte at `+N+6` is unused by JSR.

### Local variable access

```
    STA  +6,SP            ; store to local[0]
    LDA  +7,SP            ; load from local[1]
    STX  +4,SP            ; save X to its register slot (e.g. before a nested call)
```
...
```
    ...
    ... use +6..+9 for locals, +0..+5 for saved regs (read-only) ...
...
5. **PSH/PLL sequencer**: a micro‑sequence that reads/writes the 6
   register slots at fixed offsets from SP, then adjusts SP by
   `N + 6` in a single step.  No intermediate bus states are
   exposed.  Shared adder for the `SP ±= frame_size` adjustment.
...
- PSH/PLL's `SP ±= frame_size` uses the same adder; the 8-bit
  immediate is zero-extended to 12 bits for the `+ N + 6` / `- N - 6`
  calculation.

### Cycle counts (preliminary)

All SP-relative instructions are 2 bytes, 2 cycles — the stack RAM
access (read or write) happens in parallel with the operand fetch
from main memory, using the separate internal stack port.

PSH/PLL are multi-cycle but fixed-cost regardless of N:

| Instruction | Cycles | Breakdown |
|------------|--------|-----------|
| `LDA d,SP` | 2 | fetch opcode + fetch operand/read stack |
| `STA d,SP` | 2 | fetch opcode + fetch operand/write stack |
| `LDX d,SP` | 2 | |
| `LDY d,SP` | 2 | |
| `STX d,SP` | 2 | |
| `STY d,SP` | 2 | |
