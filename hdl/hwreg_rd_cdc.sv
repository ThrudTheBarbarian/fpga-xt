...
//
// Boot-to-BASIC blocker #3 (prompts/task-0003).  The SALLY->ANTIC register
...
module hwreg_rd_cdc (
    ...
    always_ff @(posedge clk_sally or posedge rst_sally) begin
        if (rst_sally) begin
            ...
            // No request in flight: disarm so the next read re-issues.
            armed    <= 1'b0;
            captured <= 1'b0;
        end else if (!armed) begin
            // New read: kick off one transaction.
            req_tog  <= ~req_tog;
            armed    <= 1'b1;
            captured <= 1'b0;
        end else if (!captured && (ack_tog_s == req_tog)) begin
            // Responder finished: latch the byte and release the stall.
            rd_data  <= resp_data_s;
            captured <= 1'b1;
        end
    end

    // Combinational so the stall engages the same cycle SALLY presents the
    // read (mirrors the banked-window busy in sally_mem).
    assign rd_busy = rd_req & ~captured;
    ...
    cdc_sync_bit #(.WIDTH(16)) u_addr_sync (
        .dst_clk (clk_sys), .src_sig (rd_addr), .dst_sig (req_addr_s));
