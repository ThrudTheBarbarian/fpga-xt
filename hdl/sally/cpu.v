...
always @*
    case( state )
        ...
        // NMOS 6502 JMP ($xxFF) page-wrap quirk: when fetching the
        // target high byte of an indirect JMP, the operand pointer
        // increments only its low byte — `{operand_hi, operand_lo + 1}`
        // wraps within the same page instead of carrying into the
        // high byte. PC at JMPI1 → JMP0 carries this NMOS-style.
        // 65C02 fixed this; the documented Atari OS doesn't trip the
        // bug, but period demos / copy-protection schemes do. See
        // docs/Issues.md `sally-jmp-indirect-bug`.
        JMPI1:          PC_temp = { DIMUX, ADD + 8'd1 };
...
always @*
    case( state )
        ...
        BRA2,
        BRK3,
        // JMPI1 deliberately omitted — its PC_temp already does the
        // NMOS-style 8-bit `ADD + 1` page-wrap above; adding PC_inc
        // would re-introduce the 16-bit carry we're trying to
        // suppress.
        JMP1,
        RTI4,
        RTS3:           PC_inc = 1;
