...
- **Companion: HID -> POKEY routing — keyboard DONE, mouse pending.**
  `motherboard/firmware/src/keymap.c` translates HID boot-protocol reports to Atari
  KBCODE. It maps **via the character**, not the key position: usage -> what a US
  keyboard would produce -> the Atari combination for that same character. The layouts
  disagree (Atari makes `:` with shift-semicolon but `[` with shift-comma, and has a
  dedicated `<`), so positional mapping would type the wrong thing. The Atari half is
  generated by inverting **TCKD**, the XL OS's own key-definition table at $FB51
  (`refs/OS-xl-rev-2-Disassembly.lst`) — which is also where the modifier bit positions
  come from: **shift is bit 6, ctrl is bit 7**. Verified on hardware with `key <usage>
  [mods]`. Events reach the FPGA as KBD_CODE/KBD_STAT with DOWN/ALLUP/BREAK, matching
  the existing $D4CF / $D4CD / $D4CB hooks in `fpga_xt_top.sv`.
  Remaining: mouse deltas, and the FPGA side consuming the registers.
