...
module cdc_fifo_1w1r #(
    ...
    // Empty when the read pointer (gray) catches up to the synchronised write
    // pointer (also gray).  NOTE: both operands MUST be in the same encoding —
    // an earlier version compared binary rd_ptr against the gray-coded
    // wr_ptr_sync, which only agreed for pointer values 0/1.  Once ≥2 entries
    // queued (e.g. the IHW register-write burst), rd_empty mis-asserted and the
    // read side over-read stale entries — re-draining an already-popped write.
    assign rd_empty = (rd_ptr_gray == wr_ptr_sync);
