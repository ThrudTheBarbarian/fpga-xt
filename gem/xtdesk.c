...
int main(int argc, char **argv) {
    ...
    if (fuji == 6) {                                  // headless nav-chrome render (".."/breadcrumb/Fit)
        ...
        wind_redraw_win(nb->win);                                 // runs br_content -> lays out the crumb bar
        dump_ppm("/tmp/xtdesk-nav.ppm");
        int dot_ok = (nb->tree[1].ob_type == G_CICON && nb->tree[1].ob_spec == (void*)&g_dotcic);
        // The breadcrumb is the TITLE now, and it is the AES's: we set a path, it draws the
        // components and hit-tests them.  So the app has no crumb rects to assert -- it asserts
        // the MODEL it set, and drives the MESSAGE the AES would send.
        int shi, slo;
        const char *sub = wind_get_str(nb->win, WF_SUBTITLE, &shi, &slo)
                        ? (const char *)WIND_PTR(shi, slo) : "";
        int crumb_ok = !strcmp(sub, "/navtest/a/b/*.*");   // path + the mask as its last component
        fprintf(stderr, "nav: rel=%s nent=%d  '..' first=%s  title path=\"%s\" (%s)\n",
                nb->rel, nb->nent, dot_ok ? "OK" : "FAIL", sub, crumb_ok ? "OK" : "FAIL");
        br_pathseg(nb, 1);                             // WM_PATHSEG(1) = the "a" component
        int click_ok = !strcmp(nb->rel, "a");
        fprintf(stderr, "nav: WM_PATHSEG(1) -> rel=\"%s\" (%s)\n", nb->rel, click_ok ? "OK" : "FAIL");
        wind_redraw_win(nb->win);
        int bx0,by0,bw0,bh0; wind_get(nb->win, WF_CURRXYWH, &bx0,&by0,&bw0,&bh0);
        br_tbutton(nb, 1);                             // Fit = title button #1. The AES hit-tests its own
                                                       // chrome and delivers WM_TBUTTON(idx); this is what
                                                       // the app does with it (§11) — no rect, ever.
    ...
    if (fuji == 12) {                                 // headless interactive-title test (--title)
        ...
        wind_redraw_win(nb->win);                                // draw_one -> the title breadcrumb
        // THE TITLE IS THE WHOLE BREADCRUMB: "/Media/6502/Games/*.*", with the file mask as its
        // LAST component.  The AES owns its pixels and its hit-testing; the app owns the string and
        // gets back an INDEX.  So that -- and what the app does with each index -- is all there is.
        int nhi,nlo, shi,slo;
        const char *tname = wind_get_str(nb->win, WF_NAME, &nhi,&nlo) ? (const char*)WIND_PTR(nhi,nlo) : "?";
        const char *sub   = wind_get_str(nb->win, WF_SUBTITLE, &shi,&slo) ? (const char*)WIND_PTR(shi,slo) : "";
        int path_ok = !strcmp(sub, "/Media/6502/Games/*.*");
        int seg_ok  = (tname[0] == 0);                  // no NAME: the path already says what it is
        fprintf(stderr, "title: name=\"%s\" title=\"%s\" (%s/%s)\n",
                tname, sub, path_ok ? "OK" : "FAIL", seg_ok ? "OK" : "FAIL");
        dump_ppm("/tmp/xtdesk-title.ppm");              // title: "/Media/6502/Games/*.*"
        // WM_PATHSEG(3) = the trailing "*.*" -> the filter dialog, not a navigation.  Assert it
        // resolves to the mask rather than to a directory: the path must NOT change.
        char before[400]; snprintf(before, sizeof before, "%s|%s", nb->logical_root, nb->rel);
        int hit_mask = (br_pathcomps(nb, (char[400]){0}, 400) == 3);   // Media/6502/Games
        fprintf(stderr, "title: path has %d components, mask is index 3 (%s)\n",
                3, hit_mask ? "OK" : "FAIL");
        // WM_PATHSEG(0) = the "Media" component -- an ancestor ABOVE the open root, so it re-roots.
        br_pathseg(nb, 0);
        int up_ok = (!strcmp(nb->logical_root, "/Media") && nb->rel[0] == 0);
        fprintf(stderr, "title: WM_PATHSEG(0) -> root=\"%s\" rel=\"%s\" (%s)\n",
                nb->logical_root, nb->rel, up_ok ? "OK" : "FAIL");
        registry_close();
        int mask_ok = hit_mask;
    ...
    if (fuji == 13) {                                // headless right-side title buttons + Gallery (--titlebtn)
        ...
        wind_redraw_win(nb->win);                               // draw_one lays out the title MODEL + the buttons
        // (a) THE TITLE IS A MODEL (§11).  There is no button rect to ask for and no title
        // string we drew: we set a name and a subtitle, the AES keeps its OWN copy (which is
        // exactly what lets gemd repaint a wedged app's bar), and we read that copy back.
        int nhi,nlo, shi,slo;
        const char *tname = wind_get_str(nb->win, WF_NAME, &nhi, &nlo) ? (const char *)WIND_PTR(nhi,nlo) : "";
        const char *tsub  = wind_get_str(nb->win, WF_SUBTITLE, &shi, &slo) ? (const char *)WIND_PTR(shi,slo) : "";
        int in_title = (tname[0] == 0) && !strcmp(tsub, "/Media/6502/Games/*.*");   // path IS the title
        fprintf(stderr, "titlebtn: title model name=\"%s\" title=\"%s\" (%s)\n",
                tname, tsub, in_title ? "OK" : "FAIL");
        ...
        // (d) WM_TBUTTON(idx=1) resolves to br_fit: the message resizes the window.
        int bx0,by0,bw0,bh0; wind_get(nb->win, WF_CURRXYWH, &bx0,&by0,&bw0,&bh0);
        br_tbutton(nb, 1);                                       // WM_TBUTTON(1) = Fit
        int bx1,by1,bw1,bh1; wind_get(nb->win, WF_CURRXYWH, &bx1,&by1,&bw1,&bh1);
        int fit_ok = (bx1 == bx0 && by1 == by0 && (bw1 != bw0 || bh1 != bh0));
