...
## P/M run slots 2 -> 8: fixed in sim, NOT yet on hardware
...
OPEN:
- No bitstream yet, so NOT hardware-verified.  Gate the build on WNS >= 0: the
  only new depth is the first-free allocation ripple and a wider obj_lit OR.  If
  it costs timing, NSLOT = 4 is a one-line fallback (Simon approved 4, wants 8
  as a failsafe) — lower TR's match count to match if so.
- Strip the TEMP DIAG8/DIAG9 ROM-window diagnostics into the SAME bitstream
  rather than spending a second 40-minute build.  The guard is USE_PS_BD in
  fpga_xt_top.sv; the SIM branch still holds the original meanings
  (antic_dbg_antic / dli_diag_word), so it is a revert of the hardware branch,
  not a deletion.  hdl/regmap/xt_gp0.json is the source of truth —
  xt_gp0_map.h AND xt_gp0_pkg.sv are both GENERATED by tools/gen_regmap.py.
  Also drop /OS/proc/romdiag in loader/test/freertos/vfs_procfs.c, which would
  otherwise report the DLI word.
- Simon has not yet confirmed the goalposts look right on the board.
