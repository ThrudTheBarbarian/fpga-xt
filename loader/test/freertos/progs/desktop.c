            ...
            if (b && aes_mode() == AES_CLIENT) {
                ...
                if (!fresh && (nw != ow || nh != oh)) {
                    ...
                    if (relayout) wind_redraw_win(b->win);
                        ...
                        // The info strip is TOP-anchored now: it never relocates on a resize,
                        // so only genuine exposure paints — a pure shrink paints NOTHING.
                        if (nw > ow) wind_redraw_rect(b->win, ow, 0, nw - ow, nh);
                        if (nh > oh) wind_redraw_rect(b->win, 0, oh, nw, nh - oh);
                    }
                }
            }
        }
