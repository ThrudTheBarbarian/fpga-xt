...
module pokey_audio #(
    ...
    // TIMER 2 WHILE THE LINE IS A MARK.  emu's underflow() opens with
    // `if (ch == 1 && timer2_held(p)) return;`, so the whole underflow EVENT is
    // suppressed -- interrupt and reload both.  Free-running instead gives the
    // same count on marks as on spaces, which is exactly what pokey_twotone
    // reports and what emu's comment names: "Free-running gives 48 in both."
    input  wire        ser_out_bit,
    ...
    // the whole UNDERFLOW EVENT, not just the counter: underflow() opens with
    // `if (ch == 1 && timer2_held(p)) return;`, so neither the interrupt nor
    // the reload happens.  Gating only the counter leaves ch2_wrap -- and so
    // timer2_pulse -- firing exactly as before.
    wire timer2_held = skctl[3] && ser_out_bit;
    ...
    // Measured on hardware across two builds:
    //   uniform N+4        -> 8-bit loops pass; 16-bit loop 2 "too early"
    //   N+7 first, both    -> 16-bit passes;    8-bit  loop 2 "too late"
