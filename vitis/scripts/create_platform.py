...
with open(_user_cmake, "w") as _f:
    _f.write("# Auto-generated by create_platform.py — TinyUSB host (ci_hs/EHCI) build.\n")
    _f.write('include_directories("${CMAKE_CURRENT_SOURCE_DIR}")\n')  # tusb_config.h, usb_hid.h
    _f.write('include_directories("%s")\n' % _tu_inc)                  # TinyUSB headers
    _f.write("set(USER_COMPILE_SOURCES\n")
    for _s in _tu_srcs:
        _f.write('  "%s"\n' % _s)
    _f.write(")\n")
print(f">> wrote TinyUSB UserConfig.cmake ({len(_tu_srcs)} sources) -> {_user_cmake}")
