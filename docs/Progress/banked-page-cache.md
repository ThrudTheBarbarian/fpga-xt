# Implementation prompt — resident page cache for the SALLY banked windows

> Status: design brief / not yet implemented. Self-contained prompt for a
> future session. Written 2026-05-22.
...
The xtc usage pattern is "map a page (`$82` code / `{$84,$83}` data), then hammer
...
Each cache holds **exactly one page** (the active `$82` / `{$84,$83}` selection),
...
- `hdl/sally_mem.sv`:
  ...
  - Snoops `$0082-$0084` into `cpu_code_bank` / `cpu_data_bank_lo` / `cpu_data_bank_hi`.
- `hdl/bank_xlat.sv` — outputs `is_in_window` (`$6000-$CFFF`), `is_code`
  (code vs data), `offset_in_block[13:0]`, `bank_id[15:0]` (code `{8'h0,$82}`,
  data `{$84,$83}`). The **page tag** is just `bank_id` (per window).
