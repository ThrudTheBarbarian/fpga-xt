...
description: "Plan for compiling xcc with itself on the Zynq hardware — the ARM-hosted route versus the 6502 soft core."
---


## Motivation

The xcc compiler is currently written in Objective-C and runs on macOS / Linux (GNUstep) or Windows (again using GNUstep).  This is fine for development, but it means:
...
The target for "on the box" compilation is a **Zynq 7020** :
...
Source .xt
  ...
  ├──── Sema 
  ├───── IR Lowering 
  ├────── [optimise] ── 6502 Lowering ── .asm
  │                   │
  │       (future)    ├── ARM64 Lowering ── .s
  │                   ├── 68k Lowering ── .s
  │                   └── ...
```
...
The critical missing piece is the ARM back-end. Although marked as 'in progress' above, that is an ARM64 port. It should be relatively easy to 
go from ARM64 to the ARM32 world for an A9 Zynq though. Everything else is
...
Should the xcc compiler look like the ObjC compiler, or be a fresh design?  There are arguments both ways.
...
**POR:** Start *structurally similar* but let the design drift
...
1. Cut over to the xcc compiler as soon as it can compile itself and
   produce working 6502 and native-platform binaries.
...
3. **Write a minimal `xcc.xt`.**  Even 200 lines of xcc that can lex
...
- `docs/optimisation` — IR-level optimiser roadmap (relevant once
