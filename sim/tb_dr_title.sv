...
module tb_dr_title;
    ...
    dl_parser u_dl (.clk(clk),.rst(rst),.start_parse(start_parse),.cold_abort(1'b0),.frame_start(1'b0),.line_start(1'b0),.prep_tick(1'b0),.vs_dli_tick(1'b1),.vs_stop_tick(1'b1),.dlistl(dlistl),.dlisth(dlisth), .dlistl_we(1'b0), .dlisth_we(1'b0),.vscrol(4'h0),
