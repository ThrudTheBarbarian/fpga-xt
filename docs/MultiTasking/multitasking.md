...
```
$0082        code bank selector (selects $6000-$9FFF, 16 KB window)
$0083        data bank selector (reserved — future)
$0084-$0085  region-C bank selector (selects $A000-$CFFF, 12 KB window)

$2400-$3FFF  System region (unbanked — kernel core, common routines)
$4000-$5FFF  Screen memory (fixed, unbanked)
$6000-$9FFF  Code bank window (via $82, 256× 16 KB = 4 MB addressable)
$A000-$CFFF  Region-C bank window (via $84/$85, 65536× 12 KB)
$D800-$FFF9  Main region (unbanked — ROM, hardware registers)
```

256 code banks × 16 KB = 4 MB of code space.
65536 region-C banks × 12 KB = 768 MB of data/heap space.
...
When the scheduler switches from process A to process B, it just writes new values into `$82` and `$84`/`$85` — the bank registers. Process B's code now appears at `$6000`; its data at `$A000`. **No relocation, no fixups, no PIC.** The bank controller *is* the MMU.
...
|--------|------|---------|----------|
| Code | 16 KB (one `$82` bank) | Yes | Executable code, read-only data |
| Data | 12 KB (one `$84/$85` bank) | Yes | Heap, globals, PCB, stack frame |
...
2      | 1    | code_bank ($82 value for this process)
3      | 2    | data_bank ($84/$85 value)
...
tick_interrupt:
    ...
    save_bank_registers_to_pcb()      // $82, $84/$85 values into PCB
...
3. Copy code into the code bank (set `$82` to the bank number, memcpy to `$6000`)
4. Copy initial data into the data page (set `$84/$85` to the page number, memcpy to `$A000`)
