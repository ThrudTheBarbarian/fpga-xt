---
title: "Palette LUT — NTSC / PAL switching"
description: "The 256-entry palette LUT mapping Atari hue/luma to RGB, with dual NTSC and PAL reference tables."
---
...
2. **Pure firmware-driven.** Leave `INIT_FILE` empty (LUT comes up
   black at /G_RST), and have the PS-side boot firmware push 256
   entries via writes to $D483-$D486 during the cold-boot sequence.
   Region toggle = re-push the other table (≈ 1 ms at 1.79 MHz bus).

We default to the synth-time bake-in since it gives a sensible
power-on display before any firmware has run.
