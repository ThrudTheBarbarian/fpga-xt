...
static const uint8_t sio_stub[] = {
    
};
...
static const uint8_t rst_stub[] = {
    ...
    0xa9,0x00,0x91,0xf0,0xc8,0xd0,0xfb,0xe6,0xf1,0xa5,0xf1,0xc9,0xa0,0xd0,0xf1,0xa2,
...
static uint8_t sio_pitch_audf(void);                                       /* fwd */
...
void xl_sio_service(volatile uint8_t *page)
{
    ...
    page[MC_OFF_SIO_AUDF]  = sio_pitch_audf();   /* the stub reads it next call */
...
/* ---- serial-bus sound ------------------------------------------------------
 * The stub makes POKEY sing for as long as a transfer is in flight (see
 * tools/xl_sio_stub.s), and the PITCH IS THE DATA RATE on a real machine -- a
 * US Doubler load sounds higher than a stock 1050.  The 6502 side must not have
 * to know what a baud is, so the A9 -- which owns the model -- hands it a ready
 * AUDF4 through the mailbox.
 *
 * POKEY's 64 kHz clock gives f = 63921 / (2 * (AUDF + 1)).  Mapping f = baud/32
 * puts a stock 19200 bus at ~600 Hz and a US Doubler at ~1.2 kHz, which is the
 * relationship the ear actually uses to tell them apart. */
static uint8_t sio_pitch_audf(void)
{
    uint32_t baud = g_siov_baud ? g_siov_baud : 19200u;   /* snappy: pitch a stock bus would have */
    uint32_t f    = baud / 32u;
    uint32_t audf = f ? (31960u / f) : 255u;
    if (audf) audf--;
    if (audf > 255u) audf = 255u;
    return (uint8_t)audf;
}

/* Publish it without disturbing the rest of the header: the mailbox is written
 * a WORD at a time, so bytes 4-7 (flags, magic) travel with it and a blind
 * write would eat a request that happened to be in flight. */
void xl_sio_publish_pitch(void)
{
    uint8_t hdr[8];
    sio_mbox_read(0, hdr, 8);
    hdr[MC_OFF_SIO_AUDF] = sio_pitch_audf();
    sio_mbox_write(0, hdr, 8);
}
...
int xl_boot(const char *path, int drive)
{
    ...
    /* The stub reads the bus-tone pitch out of the mailbox BEFORE it rings the
     * doorbell, so the very first sector of this load needs it there already. */
