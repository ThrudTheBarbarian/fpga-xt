...
2. **No explicit random access** — you can only PUSH and PULL. Accessing a parameter or local variable requires `TSX` + `LDA $0100,X`, which is slow and ties up X.
...
This memory is **only partially** mapped into the 16-bit address space — normal `LDA`/`STA` can only see 256 bytes of it.  Only stack operations (PHA, PLA, JSR, RTS, RTI, BRK) and the new SP-relative instructions access it.
...
6502 supported without (typically) crashing.  Code that never pushes deeper than 256 bytes sees no difference.

For depths >256 bytes, `TSX` still returns `SP[7:0]` (the low 8 bits),
but `LDA $0100,X` now reads the first-256-byte window rather than
...
Being able to address the stack directly does two things - it makes using the stack more viable for the compiler, and it effectively gives a huge local storage space in ZP, because of the speed of the operations.
...
almost any function-call frame.
...
| Opcode | Mnemonic | Operation | JAM? |
|--------|----------|-----------|------|
| `$89` | `BIT #imm8` | N,Z,V set if applicable | no, but NOP |
...
`d,SP` (§2) covers *scalar* in-frame access, but a compiler still
...
1. **Dereferencing a pointer that lives on the stack.**  With only `d,SP`
   you can load the pointer's bytes but not follow them — so pointer-type
...
`(d,SP),Y` is functionally aligned with `(zp),Y` - the 16-bit pointer is just fetched from the hidden stack instead of zero page; the post-index by `Y` and the resulting access hit **main memory**.  `d,SP,X` is `d,SP` with `X` added
into the (clamped, 12-bit) stack address — the access stays in the hidden stack.
...
group on NMOS.  A documented-only core such as this one decodes *none* of it, and SALLY's other embellishments all live in cc=00 (`$x4`: PUSH/POP) and
...
four-way match. The remaining twelve `$x3` slots are reserved for future SP-indirect variants (`ADC (d,SP),Y`, `CMP d,SP,X`, …).
...
When implementing a function call, the function prologue needs to:
...
The 6502 SP invariant is that **SP points at the next free byte** — every
push writes at `SP`, then decrements SP.  So `SP+0` is always the next push
target, by definition; that's not a property of the frame layout.

The interesting design choice is where PSH puts **saved P**.  The natural
spot — `SP+0`, against the new SP — is wrong: the first stray push inside
the function body (`PHA`/`PHP`, `PUSH X/Y`, `JSR`, or an IRQ/NMI/BRK)
would overwrite it.  So PSH places saved P at `SP+1` and leaves `SP+0`
deliberately empty.  That one-byte gap between SP and the lowest occupied
slot is the **guard byte** — it's not doing anything as a byte, it's the
*absence* of an allocation that matters.  A stray push lands on the empty
slot instead of on saved P; a JSR/IRQ that pushes 2–3 bytes spills the rest
below the frame onto bytes that don't matter (the call/IRQ returns and
cleans them up).

Without that one byte of padding, a single push corrupts the saved-register
area and defeats the interrupt-safety guarantee — see
[issue #0001](/project/issues/psh-guard-byte/) for the off-by-one the
implementation originally shipped with.
...
```
frame_size = N + 7               ; guard(1) + regs(6) + locals(N)
SP = SP − frame_size             ; 1. allocate full frame

stack8(SP + 6) = A               ; 2 saved A
stack8(SP + 5) = X               ; 3 saved X
stack8(SP + 4) = Y               ; 4 saved Y
stack8(SP + 3) = SP_hi_entry     ; 5 saved SP[11:8] (entry value)
stack8(SP + 2) = SP_lo_entry     ; 6 saved SP[7:0]  (entry value)
stack8(SP + 1) = P               ; 7 saved P

; stack8(SP + 0) is the guard byte — left unwritten.
; SP is unchanged by the register writes, so it still
; points at the guard byte.
```
...
higher addresses
         ...
         │    saved P          │  ← at SP + 1
         ├─────────────────────┤
    SP → │    guard byte       │  ← at SP + 0 (free / irq safe)
         └─────────────────────┘
lower addresses
```
...
The **gap byte** at `+N + 7` is incidental — it sits at the address SP
held *before* PSH ran (call that `entry_SP`, the free byte left by the
preceding `JSR`).  PSH adjusts SP down by `N + 7` and fills the frame
upward from the new SP, so locals run from `+7` to `+N + 6` and the byte
at `+N + 7` is simply left over — never allocated, never written.

Unlike the **guard byte** at `+0`, which exists deliberately to absorb a
stray push inside the frame, the gap is just a geometric side effect of
keeping local offsets fixed at `+7..+N+6` regardless of `N`.  Both bytes
are "free" in the sense that PSH doesn't touch them, but only the guard
byte plays a functional role.
...
```
        ...
        ...           ; (caller resumes here after func returns)

func:   PSH  #12      ; first instruction inside func — allocates the frame
```
...
$FEB:  saved SP_hi       ← SP + 3  (holds SP[11:8] of entry $FFB)
$FEC:  saved Y           ← SP + 4
$FED:  saved X           ← SP + 5
$FEE:  saved A           ← SP + 6
$FEF:  local[0]          ← SP + 7
...
SP    = SP + (N + 7)          ; 7. dealloc full frame (inc guard)
```
...
locals, and the gap byte at `+N+7` (the entry SP) is unused by PSH.
...
```
    ...
    JSR  my_func
    ; A = return value
    ADD  SP, #2           ; discard both param bytes — A and flags untouched
```

This is the standard cdecl-style caller cleanup.  `ADD SP, #signed8`
(opcode `$22`, §2 "Stack adjustment") drops the pushed parameters in a
single 2-cycle opcode without touching A or the flags — so the return
value in A survives the cleanup directly, with no save/restore dance.
On a stock 6502 (no `ADD SP`) the equivalent is the four-instruction
sequence `STA -1,SP : PLA : PLA : LDA +1,SP`.
...
- The signed adder `SP + d` → clamp → stack-RAM address must settle
  within one clock cycle.  The 12-bit signed add and the two clamp
  comparators are cheap on their own (well under 4 ns), but at the
  ~120 MHz SALLY clock (≈8.3 ns period) the path that has to close is
  the whole sequence in one fabric cycle: `SP + d` → clamp → stack-RAM
  read → ALU.  That makes it a real timing path, not a free one — it
  closes at 120 MHz only with the effective address computed early
  (form `SP + d` in the decode cycle so the clamp and BRAM access fit
  the following one).
...
$x2 column (all JAM on original NMOS 6502):
  ...
  $02: STX d,SP        $42: LDX d,SP        
  $12: STY d,SP        $52: LDY d,SP        
  $82: (reserved)      $C2: (reserved)
  $92: STA d,SP        $D2: CMP d,SP
  $22: ADD SP, #imm    $62: PLL #N         
  $A2: *LDX #imm*      $E2: (reserved)
  $32: PSH #N          $72: ADC d,SP       
  $B2: LDA d,SP        $F2: SBC d,SP

* $A2 is LDX #imm on the original 6502 (NOT a JAM) 
  — do not reassign.
...
$x3 column (cc=11 combo-illegals on NMOS; whole column free in SALLY):
  ...
  $03: LDA (d,SP),Y    $13: STA (d,SP),Y    
  $23: LDA d,SP,X      $33: STA d,SP,X
...
- **New code** targeting the 'xt' model should not use `TSX`/`TXS`.
  Use SP-relative addressing (`LDA d,SP`, `STA d,SP`, etc.) instead. This will generally be enforced by the compiler/assembler
  
- **Old code** (Atari OS, ROM routines) is unaffected — it runs within
  the 256-byte-compatible aliased window and never hits the new
  opcodes.
  
- **PSH/PLL** (frame setup/teardown) and `ADD SP, #signed8` (caller-side
  parameter cleanup, alloca-style adjustment) are the supported ways to
  adjust SP in new code — all clamp to the 12-bit range and keep the
  alias-model invariant.  Direct manipulation of SP via `TXS` breaks
  that invariant and may confuse the frame layout.
