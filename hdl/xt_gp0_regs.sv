...
module xt_gp0_regs (
    ...
    // CONTINUOUS trace -> DDR (xt_trace_axi): never halts the core, so it is
    // the only trace usable while the virtual SIO drive is live.
    output reg  [31:0] dbg_dtr_ctrl,      // [0]=enable
    output reg  [31:0] dbg_dtr_base,      // DDR ring base (128B aligned)
    output reg  [31:0] dbg_dtr_mask,      // ring size-1, power of two
    input  wire [31:0] dbg_dtr_wrote,     // bytes written since enable
    input  wire [31:0] dbg_dtr_drops,     // entries lost to a full FIFO
                        ...
                        unique case (aw_blk)
                            ...
                            BLK_DEBUG: begin
                                unique case (aw_off)
                                    ...
                                    DBG_DTR_BASE:   dbg_dtr_base   <= w_data;
                                    DBG_DTR_MASK:   dbg_dtr_mask   <= w_data;
                        ...
                        unique case (ar_blk)
                            ...
                            BLK_DEBUG:
                                unique case (ar_off)
                                    ...
                                    DBG_DTR_WROTE:  s_axi_rdata <= dbg_dtr_wrote;
                                    DBG_DTR_DROPS:  s_axi_rdata <= dbg_dtr_drops;
