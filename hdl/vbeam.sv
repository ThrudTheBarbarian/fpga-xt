...
// generation. Industry-standard VESA timings.
//
// Selectable output mode at parameter level (640×480@60 or 800×600@60).
// At runtime, the OUTPUT_MODE register flips between the two by
// instantiating both with shared output mux and a vsync-aligned switch.
// For M3 we instantiate a single mode at a time; M14 adds the runtime
// flip.
//
// Atari-row mapping (ANTIC-compat): 192 active ANTIC lines start
// after a top letterbox of (V_ACTIVE - 384)/2 native lines and run
// for 384 native lines (line-doubled). The atari_row output gives
// the 0..191 ANTIC line for the current native scan line, or 0xFFFF
// during letterbox / blanking.

`default_nettype none

module vbeam #(
    // 640×480@60 VESA timing.
