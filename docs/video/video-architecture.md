...
duplicate fetches, so its read traffic is the surface's, not the screen's.
A scale-1 plane fetches every scanline, the pattern `plane_fetch` sustains
for one plane on HP0.
