...
- [ ] **PL-only test-pattern mux in `plane_compositor`** — a
      build parameter that bypasses the plane_fetch DDR3 reads and
      drives a deterministic gradient / colour-bar split straight
      to the 565 output, so Phase 2 can prove the HDMI chain with
      no PS and no DDR3 traffic.  Not yet present in the plane
      compositor (the old `SCANOUT_TEST_PATTERN` lived only in the
      orphaned `fb_scanout.sv`).  Add before the Phase 2 step, or
      fold Phase 2 into Phase 3 and fill DDR3 from the PS instead
      (see Phase 2).
...
Approach: the display path is `vbeam` (1080p60 raster) →
`plane_fetch` ×N (per-plane DDR3 line read) → `plane_compositor`
(depth/scale/clip mix + 8888→565 Bayer dither) → `sprite_engine`
→ the RGB565 + DE + HSYNC + VSYNC pins.  For a PL-only first test
(no PS, no DDR3 traffic) there are two routes:

- **Add a test-pattern mux to `plane_compositor`** (RTL-adds
  prereq, not yet done): a build parameter that drives a
  deterministic gradient / colour-bar split straight to the 565
  output, bypassing the plane_fetch DDR3 reads.  This is the true
  PL-only test — preferred for isolating the HDMI chain from DDR3.
- **Fold Phase 2 into Phase 3** instead: bring up the PS first,
  have it fill the plane-0 framebuffer at `0x3000_0000` (stride
  8192) with a gradient, and let `plane_fetch0` + the compositor
  display it.  Exercises more of the stack at once, at the cost of
  no longer isolating the HDMI path from DDR3 + AXI HP0.

The SiI9022A I²C init is handled **in the PL** — `hdmi_config`
(`fpga_xt_top.sv`) bit-bangs the SiI9022A TPI registers for
1080p60 over the open-drain I²C bus and raises a `done` flag; no
FSBL/PS step is required to bring the transmitter out of reset.
...
- Colours wrong — RGB565 bit order mismatch.  The mapping in
  ...
  the compositor's `rgb_r/g/b` output widths (5/6/5, via
  `sprite_engine` to the pins) match.
...
## Phase 6 — plane compositor reads DDR3, drives HDMI

If Phase 2 used the PL-only test-pattern mux, drop it now so
`plane_fetch0` reads the real plane-0 framebuffer (`0x3000_0000`,
stride 8192) over AXI HP0.  The HDMI output should reflect
whatever the blitter has written to DDR3.  (If Phase 2 was folded
into Phase 3 — PS filling DDR3 — this is already live; here you
just confirm the blitter's writes appear.)
...
**Likely fail**:
- Black screen — `plane_fetch0`'s AXI HP0 read isn't completing
  in time.  Probe its line-buffer fill state machine; check the
  burst arrived before the line started reading from the
  off-buffer.  Confirm the plane-0 `surface_base`/`stride_bytes`
  match where the blitter wrote (`0x3000_0000`, stride 8192).
- Wrong colours — `plane_compositor`'s 8888→565 dither is wrong.
  Should drop alpha + Bayer-dither R/G/B down to 5/6/5.  An
  off-by-one in the bit positions shows as a colour cast.
- Tearing — the plane_fetch ping-pong line-buffer handoff isn't
  aligned with HSYNC.  Should be safe because the read side is on
  `clk_pix` and the swap happens at HSYNC, but worth eyeballing.
...
ANTIC pulls display-list bytes from sally_mem (snoop mode); its
per-row compositor builds Atari scanlines and writes them back to
the plane-1 DDR3 surface; `plane_fetch1` + `plane_compositor`
bring that plane in alongside the desktop and drive HDMI; SALLY
runs a small Atari demo.  Audio out is a separate sub-track on
POKEY → I²S → HDMI audio islands.
